...
var main = function (input) {
  ...
  var currentIndex = 0;
  if (mode == "start") {
    myOutputValue = `You have ${playerPoints} points, how much do you want to bet?`;
    if (playerPoints == 0) {
      myOutputValue = `You have zero points.<br><br>GAME OVER<br>${gameoverImage}<br>Please refresh the page to play again.`;
    } else if (input <= playerPoints && input > 0) {
      bet = Number(input);
      myOutputValue = `You have placed ${input} points as your bet!`;
      mode = "first 2 cards";
    }
  } else if (mode == "first 2 cards") {
    ...
    // //Force Win
    // computerHand = [
    //   { name: "9", valueofCards: 9 },
    //   { name: "King", valueofCards: 10 },
    //   { name: "2", valueofCards: 2 },
    // ];
    ...
    myOutputValue = myOutputValue + `<br>You have ${playerPoints} points now`;
    mode = "start";
    playerHand = [];
    computerHand = [];
    cardDeck = makeDeck();
